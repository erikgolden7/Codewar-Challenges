...
///
// Original Solution (To slow for code wars)
///
...
function gap(gap, start, end) {
  ...
  primeNumbers.forEach((prime, index) => {
    if (prime + gap === primeNumbers[index + 1]) {
      final.push(prime, primeNumbers[index + 1]);
    }
  });

  if (final.length > 0) return final.slice(0, 2);
  return null;
}
